...
"""Asll Pay telegram menu package."""
import os
import asyncio
import datetime
import json
...
class AccountsMenuMessage(BaseMessage):
    ...
    def __init__(self, navigation: MyNavigationHandler, update_callback: Optional[List[UpdateCallback]] = None) -> None:
        ...
        self.add_button(new_row=True, label="⬅️", callback=navigation.goto_back)
        self.add_button(label="🏡", callback=navigation.goto_home)
        # self.add_button_back()
        # self.add_button("Back2", callback=navigation.goto_back)
        # self.add_button_home()
        if update_callback:
    ...
    def update(self) -> str:
        """Update message content."""
        with open(os.path.join(ROOT_FOLDER, "resources", "payments.txt"), "r", encoding="utf-8") as f:
            return f.read()
...
class ServicesMenuMessage(BaseMessage):
    ...
    LABEL = "services_message"

    def __init__(self, navigation: MyNavigationHandler, update_callback: Optional[List[UpdateCallback]] = None) -> None:
        """Init SecondMenuMessage class."""
        super().__init__(
            navigation,
            ServicesMenuMessage.LABEL,
            notification=False,
            # picture=(ROOT_FOLDER / "resources" / "packages.png").resolve().as_posix(),
            expiry_period=datetime.timedelta(seconds=5),
            input_field="Enter an option",
        ...
        account = AccountsMenuMessage(navigation, update_callback)
        payments = PaymentsMenuMessage(navigation, update_callback)
        self.add_button(label="💳گیفت کارت ها", callback=gift_card, new_row=True)
        self.add_button(label="🏦حساب های بین المللی", callback=account, )
    ...
    def update(self) -> str:
        """Update message content."""
        return "خدمت خود را انتخاب کنید."


class LearningMenuMessage(BaseMessage):
    LABEL = "آموزش و راهنما 📚"
    def __init__(self, navigation, updatecallback=None):
        super().__init__(navigation, self.LABEL, notification=False)
        self.addbutton("آموزش خرید", callback=self.action_buy_guide)
        self.addbutton("آموزش امنیت", callback=self.action_security_guide)
        self.addbutton("⬅️ بازگشت", callback=navigation.gotoback)
        self.addbutton("🏠 خانه", callback=navigation.gotohome)
    def action_buy_guide(self, *args): self.notify("برای خرید کافیست سرویس موردنظر را انتخاب، ثبت سفارش و پرداخت کنید.")
    def action_security_guide(self, *args): self.notify("حتماً راهنماهای امنیتی را مطالعه کنید تا حساب شما امن بماند.")
...
class ContactMenuMessage(BaseMessage):
    ...
    def __init__(self, navigation, updatecallback=None):
        super().__init__(navigation, self.LABEL, notification=False)
        self.addbutton("ارسال پیام به پشتیبانی", callback=self.action_contact)
        self.addbutton("⬅️ بازگشت", callback=navigation.gotoback)
        self.addbutton("🏠 خانه", callback=navigation.gotohome)
    def action_contact(self, *args): self.notify("پیام شما به پشتیبانی ارسال شد.")
...
class StartMessage(BaseMessage):
    ...
    def __init__(self, navigation: MyNavigationHandler, message_args: Optional[List[UpdateCallback]] = None) -> None:
        ...
        services = ServicesMenuMessage(navigation, update_callback=message_args)
        self.add_button("خدمات ما 🛠️", callback=services)
        ...
        # define menu buttons
        # self.add_button(label="آموزش", callback=learnings)
        # self.add_button(label="لیست خدمات", callback=services)
        # action_message = ActionAppMessage(navigation)
        # self.add_button(label="Action", callback=self.webapp_cb, web_app_url=self.URL)
